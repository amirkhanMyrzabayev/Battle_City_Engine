...
namespace RenderEngine
{
		...
		, m_lastFrameId(0)
	{
		...
		m_vertexCoordBuffer.init(&vertexCoords, sizeof(vertexCoords));
		VertexBufferLayout vertexCoordsLayout;
		vertexCoordsLayout.addElementLayoutFloat(2, GL_FALSE);
		m_vertexArray.addBuffer(m_vertexCoordBuffer, vertexCoordsLayout);

		m_textureCoordBuffer.init(&textureCoords, sizeof(textureCoords));
		VertexBufferLayout textureCoordsLayout;
		textureCoordsLayout.addElementLayoutFloat(2, GL_FALSE);
		m_vertexArray.addBuffer(m_textureCoordBuffer, textureCoordsLayout);
						...
						const size_t frameId) const
	{
		if (m_lastFrameId != frameId)
		{
			...
			m_textureCoordBuffer.update(&textureCoords, sizeof(textureCoords));
		}
