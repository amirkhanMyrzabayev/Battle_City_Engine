...
Bullet::Bullet( const double velocity,
	...
	, m_maxVeclocity(velocity)
	, m_isActive(false)
	, m_isExplosion(false)
{
	

	auto onCollisionCallback = [&](const IGameObject& object, Physics::ECollisionDirection)
		{
			setVelocity(0);
			m_isExplosion = true;
...
void Bullet::render() const
{
	...
	if (m_isExplosion)
	{
		...
		case Bullet::EOrientation::Top:
			m_pSprite_explosion->render(m_position - m_explosionOffset + glm::vec2(0, m_size.y / 2.f), m_explosionSize, m_rotation, 
										m_layer + 0.1f, m_spriteAnimator_explosion.getCurrentFrame());
			break;
		...
		case Bullet::EOrientation::Left:
			m_pSprite_explosion->render(m_position - m_explosionOffset - glm::vec2(m_size.x / 2.f, 0), m_explosionSize, m_rotation,
										m_layer + 0.1f, m_spriteAnimator_explosion.getCurrentFrame());
			break;
...
void Bullet::fire(const glm::vec2& position, glm::vec2& direction) {
