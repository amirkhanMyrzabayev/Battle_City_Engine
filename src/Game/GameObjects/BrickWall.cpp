						...
						glm::vec2(m_size.x / 2.f, 0) }
{
	...
	case EBrickWallType::All:
		...
		m_brickLocationToColliderMap[static_cast<size_t>(EBrickLocation::TopLeft)] = &m_colliders.emplace_back(getAABBForBrickState(EBrickLocation::TopLeft,
																																	EBrickState::All, m_size), 
																																	onCollisionCallbackTopLeft);
		m_brickLocationToColliderMap[static_cast<size_t>(EBrickLocation::TopRight)] = &m_colliders.emplace_back(getAABBForBrickState(EBrickLocation::TopRight,
																																	EBrickState::All, m_size),
																																	onCollisionCallbackTopRight);
		m_brickLocationToColliderMap[static_cast<size_t>(EBrickLocation::BottomLeft)] = &m_colliders.emplace_back(getAABBForBrickState(EBrickLocation::BottomLeft,
																																		EBrickState::All, m_size),
																																		onCollisionCallbackBottomLeft);
		m_brickLocationToColliderMap[static_cast<size_t>(EBrickLocation::BottomRight)] = &m_colliders.emplace_back(getAABBForBrickState(EBrickLocation::BottomRight,
																																		EBrickState::All, m_size),
...
BrickWall::EBrickState BrickWall::getBricksAfterCollision(const EBrickState currentState, const Physics::ECollisionDirection direction)
{
	switch (currentState)
	{
...
void BrickWall::onCollisionCallback(const EBrickLocation location, const IGameObject& object, const Physics::ECollisionDirection direction)
{
	...
	const EBrickState newBrickState = getBricksAfterCollision(m_eCurrentBrickState[static_cast<size_t>(location)], direction);
