...
std::shared_ptr<RenderEngine::Sprite> getSpriteForDescription(const char description)
{
	...
	case 'F':
		return nullptr;
	default:
		std::cerr << "Unknown Start level description" << std::endl;
	}
	return nullptr;
}

StartScreen::StartScreen(const std::vector<std::string>& startScreenDesription, Game* pGame)
	: m_pGame(pGame)
...
{
	if (startScreenDesription.empty())
	{
		std::cerr << "Empty Start description" << std::endl;
	}
	...
	for (const std::string& currentRow : startScreenDesription)
	{
		unsigned int currentLeftOffset = leftOffsetPixels;
		for (const char currentElement : currentRow)
		{
			m_sprites.emplace_back(getSpriteForDescription(currentElement), glm::vec2(currentLeftOffset, currentBottomOffset));
...
void StartScreen::processInput(std::array<bool, 349>& keys)
{
	...
	if (keys[GLFW_KEY_ENTER])
	{
		...
		{
		case 0:
			m_pGame->startNewLevel(0, Game::EGameMode::SinglePLayer);
			break;
		case 1:
